...
public class Warehouse {
    ...
    private static Map<String, Map<String, Integer>> inventoryAllocator(Map<String, Integer> itemsOrdered,
            Map<String, Map<String, Integer>> warehouseInventory) {
                ...
                for (Map.Entry<String, Map<String, Integer>> warehouseItem : warehouseInventory.entrySet()) {
                    ...
                    if (warehouseItem.getValue().get(ordered_item.getKey()) != null) {
                        if (remainingNumNeeded <= warehouseItem.getValue().get(ordered_item.getKey())) {
                            ...
                            remainingNumNeeded = remainingNumNeeded
                                    - warehouseItem.getValue().get(ordered_item.getKey());
                            resultMap.get(warehouseItem.getKey()).put(ordered_item.getKey(),
                                    warehouseItem.getValue().get(ordered_item.getKey()));
                        }
                    }
                }
